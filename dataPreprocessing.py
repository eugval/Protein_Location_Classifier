...
#
save_suffix = "_window"
#x = vectorizer.fit_transform([global_feature_dict(rec,False) for rec in x_raw])
#x = vectorizer.fit_transform([global_feature_dict(rec,True) for rec in x_raw])
x = vectorizer.fit_transform([global_and_sliding_window_feature_dict(rec,100,False) for rec in x_raw])



X_train, X_test, y_train, y_test = train_test_split( x, y_true, test_size=0.1)

with open("Saved_Data/Train_Examples{}.pickle".format(save_suffix),'wb') as f:
    pickle.dump(X_train,f)


with open("Saved_Data/Train_Labels{}.pickle".format(save_suffix),'wb') as f:
    pickle.dump(y_train,f)

with open("Saved_Data/Test_Examples{}.pickle".format(save_suffix),'wb') as f:
    pickle.dump(X_test,f)

with open("Saved_Data/Test_Labels{}.pickle".format(save_suffix),'wb') as f:
